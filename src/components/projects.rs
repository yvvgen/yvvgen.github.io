...
pub fn projects() -> Html {
    ...
    html! {
        <section id="projects" class="py-20 bg-base-100 text-base-content">
            <h2 class="text-5xl font-extrabold text-center mb-16 text-neon-primary">{"Featured Projects"}</h2>
                    ...
                    projects.iter().map(|project| html! {
                        <div class="card-synthwave shadow-xl hover:shadow-2xl transition duration-500 transform hover:-translate-y-2">
                            <div class="card-body">
                                ...
                                <p class="text-lg text-base-content/80">{ &project.description }</p>
                                <div class="card-actions justify-end mt-4">
                                    {
                                        project.tags.iter().map(|tag| html! {
                                            <div class="badge badge-secondary cursor-default">
                                                { tag }
                                            </div>
                                        }).collect::<Html>()
                                    }
                                </div>
                                <a href="#" class="mt-4 link link-hover text-sm text-accent font-medium hover:text-neon-accent">{"View Source »"}</a>
                            </div>
                        </div>
                    }).collect::<Html>()
                }
